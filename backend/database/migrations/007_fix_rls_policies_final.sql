-- Исправление RLS политик для Supabase безопасности (финальная версия)
...
-- Политики для rag_dialogs (оба поля приводятся к text)
...
-- Политики для provider_configs (оба поля приводятся к text)

...
┌────────────────────────────▼────────────────────────────────────┐
│                     BACKEND (FastAPI)                            │
│  Порт: 8080 | 145+ эндпоинтов | Supabase (PostgreSQL)           │
│  Auth | Keys | Chat | Providers | Memory | Analytics            │
└────────────────────────────┬────────────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────────────┐
│                   LITELLM SERVICE                                │
│         20+ провайдеров через единый интерфейс                   │
│  GigaChat | Groq | OpenAI | Google | Anthropic | OpenRouter     │
└─────────────────────────────────────────────────────────────────┘
```
...
└─────┬───────┘
      ▼
┌─────────────┐
│  Generate   │ ← LiteLLM (20+ провайдеров)
└─────┬───────┘
      ▼
┌─────────────┐
...
|-----|-----------|----------|
| **RAG** | ChromaDB | Семантическая память диалогов |
| **Episodic** | SQLite | Эпизоды с временными метками |
| **Semantic** | SQLite | Общие знания и концепции |
| **Facts** | SQLite + ChromaDB | Структурированные факты |
...
Через **LiteLLM** + прямой SDK для GigaChat:
...
| Файл | Данные |
|------|--------|
| `data/chroma/` | RAG векторы (ChromaDB) |
